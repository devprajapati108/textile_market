...
// Helper to convert FormData entries to object
const parseFormData = async (formData: FormData) => {
  ...
  for (const [key, value] of entries) {
    if (key === 'images') {
      ...
      data.images.push(`/uploads/${filename}`);
    } else if (key === 'specifications') {
      try {
        data.specifications = JSON.parse(value as string);
      } catch {
        data.specifications = [];
      }
    } else {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const products = await Product.find({ userId: decoded.userId });
...
export async function POST(req: Request) {
  try {
    ...
    const { valid, decoded } = verifyToken(token);
    console.log("token, decoded userId: in --------->", decoded?.userId);
    if (!valid || !decoded?.userId) {
      console.log("Invalid token, decoded userId: ", decoded?.userId);
      return NextResponse.json({ message: 'Invalid token' }, { status: 403 });
    }

    // Parse FormData
    const formData = await req.formData();
    const productData = await parseFormData(formData);
    productData.userId = decoded.userId;
    productData.userRole = decoded.role; // ✅ add this line
    // Create Product in DB
    const newProduct = await Product.create(productData);
    console.log("Product created by userId:", productData.userId);
...
// POST: Create a product with image and token auth
// export async function POST(req: Request) {
//   try {
//     await connectDB();
...
//     const { valid, decoded } = verifyToken(token);
//     if (!valid || !decoded?.userId) {
//       console.log("in invilad token---------->",decoded?.userId);
//       return NextResponse.json({ message: 'Invalid token' }, { status: 403 });
//     }
...
//     const newProduct = await Product.create(productData);
//     console.log("user id is:-------------->",productData.userId)
//     return NextResponse.json({ message: 'Product created', product: newProduct }, { status: 201 });
//   } catch (error: any) {
//     return NextResponse.json({ message: error.message }, { status: 500 });
//   }
